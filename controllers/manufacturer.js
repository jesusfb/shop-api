...
const manufacturerController = {
  ...
  byId(req, res) {
    ...
    Manufacturer.findOne({ _id: idParam }).exec((err, manufacturer) =>
  ...
  create(req, res) {
    ...
    newManufacturer.save((err, saved) => {
      if (saved) {
        Manufacturer.findOne({ _id: saved._id }).exec((err, manufacturer) =>
          res.json(manufacturer)
        );
      }
    });
  ...
  remove(req, res) {
    const idParam = req.params.id;
    Manufacturer.findOne({ _id: idParam }).remove((err, removed) =>
      Product.find({
        // remove products too
        manufacturer: idParam
